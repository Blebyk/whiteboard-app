...
const Canvas = forwardRef<CanvasRef, CanvasProps>(function Canvas(props, ref) {
  ...
  useEffect(() => {
    ...
    import('fabric').then(async (fab) => {
      ...
      // ── Selection events ────────────────────────────────────
      canvas.on('selection:created', () => pRef.current.onSelectionChange(true));
      canvas.on('selection:updated', () => pRef.current.onSelectionChange(true));
