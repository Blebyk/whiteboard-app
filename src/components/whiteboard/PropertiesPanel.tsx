...
export default function PropertiesPanel({
  ...
  const [strokeCustom, setStrokeCustom] = useState(
    strokeColor.startsWith('#') ? strokeColor : '#1a1a2e'
  );
    ...
    >
      ...
      {showStroke && (
        <Section title="Цвет контура">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <ColorPicker
              value={strokeColor.startsWith('#') ? strokeColor : strokeCustom}
              onChange={(hex) => { setStrokeCustom(hex); onStrokeColorChange(hex); }}
            />
      ...
      {showFill && (
        <Section title="Заливка">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <ColorPicker
              value={fillColor.startsWith('#') ? fillColor : fillCustom}
              onChange={(hex) => { setFillCustom(hex); onFillColorChange(hex); }}
            />
            <span style={{ fontSize: '11px', color: '#9ca3af', fontFamily: 'monospace' }}>
