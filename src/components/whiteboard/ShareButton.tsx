...
export default function ShareButton({ boardId }: ShareButtonProps) {
  ...
  // Load current shared users when opening
  useEffect(() => {
    if (!open) return;
    setLoadingList(true);
  ...
  async function handleShare(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Не удалось поделиться');
        return;
      }
      ...
      // Refresh user list
      setUsers((prev) =>
        prev ? [{ ...data.user, created_at: new Date().toISOString() }, ...prev.filter((u) => u.id !== data.user.id)] : null
      );
    } catch {
  ...
  async function revoke(userId: number) {
    try {
      const res = await fetch(`/api/boards/${boardId}/share?userId=${userId}`, {
        method: 'DELETE',
      });
      if (res.ok) {
